...
class WPToolset_Field_Credfile extends WPToolset_Field_Textfield {
	...
	public function init() {
		...
		// We need to manually craft the ajaxutl on demand because of two reasons:
		// - first, we are using a custom AJAX solution, not the native WordPress one
		// - second, CRED_ABSURL is crafted before WPML can filter plugins_url() so on sites 
		//   with different domains per language the ajaxurl would point to the main site, 
		//   firing a cross domain request error.
		$credfile_ajaxurl = plugins_url() . '/' . basename( CRED_ABSPATH );
		$credfile_ajaxurl = untrailingslashit( $credfile_ajaxurl ) . '/application/submit.php';

		wp_localize_script(
			CRED_Asset_Manager::AJAX_FILE_UPLOADER, 'settings', array(
				'media_settings' => self::get_image_sizes( 'thumbnail' ),
				'ajaxurl' => $credfile_ajaxurl,
