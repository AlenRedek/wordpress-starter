...
class CRED_Translate_Field_Factory {
	...
	public function cred_translate_form_name( $name, &$field ) {
		...
		return array( "cred_form_" . CRED_StaticClass::$out['prg_id'] . $result_field . $count );
	}
	...
	public function cred_translate_field( $name, &$field, $additional_options = array() ) {
		...
		$zebraForm = $this->_formBuilder->get_form_rendering();
		...
		if ( ! $is_tax ) {
			...
			$field_configuration_args = array(
				...
				'can_accept_empty_preset_value' => ( isset( $field['cred_generic'] ) && $field['cred_generic'] && $zebraForm->is_submit_success ),
			...
			$field_result = $zebraForm->add( $type, $name, $value, $attributes, $field );
			...
			if ( ! array_key_exists( 'master_taxonomy', $field ) ) { // taxonomy field
				...
				$field_result = $zebraForm->add( ! $field['hierarchical'] ? 'taxonomy' : 'taxonomyhierarchical', $name, $value, $field_configuration );
