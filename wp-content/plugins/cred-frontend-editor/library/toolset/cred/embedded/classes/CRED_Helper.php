...
//                                __linkel__.innerText = _cred_cred_delete_post_handler.refs[__link__];
	...
	public static function removeCREDButton( $cred_button, $context ) {
		...
		if (
			isset( $_GET['page'] ) 
			&& (
				// Remove CRED button from Gravity Forms pages.
				'gf_edit_forms' === $_GET['page'] 
				// Remove CRED button from the e-popotheme settings page, provided by Redux.
				|| (
					'_options' === $_GET['page'] 
					&& in_array( $context, array( 'custom_copyright', 'custom_power' ) )
				)
			)
		) {
			return false;
