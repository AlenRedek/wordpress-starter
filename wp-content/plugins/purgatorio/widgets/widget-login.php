...
class pg_login_widget extends WP_Widget {

    public function __construct(){
        $widget_details = array(
            'classname' => 'pg_login_widget',
            'description' => __('Login form with redirection to selected page.', 'purgatorio')
        );

        parent::__construct( 'pg_login_widget', __('[Purgatorio] Login form', 'purgatorio'), $widget_details );
    }
    ...
    public function form( $instance ){
        ...
        <p>
            <label for="<?php echo $this->get_field_name( 'page_id' ); ?>"><?php _e( 'Redirect page', 'purgatorio' ); ?></label>
    ...
    public function widget( $args, $instance ) {
        ...
        <?php
            ...
            if(is_user_logged_in()){
                ...
                echo '<p>'.__('You are currently logged in as:', 'purgatorio') . '<br />' . $user->data->display_name.'</p>';
                echo '<p><a href="'.wp_logout_url( home_url() ).'" class="btn btn-primary">'.__('Logout', 'purgatorio').'</a></p>';
            }else{ ?>
                ...
                <form name="loginform" id="loginform" action="<?php echo esc_url( wp_login_url(get_permalink($redirect_page->ID)) ); ?>" method="post" _lpchecked="1">
                	...
                	<p class="login-submit form-group">
                		<input type="submit" name="wp-submit" id="wp-submit" class="btn btn-primary" value="<?php _e('Log in','purgatorio'); ?>">
                	</p>
