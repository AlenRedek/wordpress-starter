...
class Toolset_Framework_Installer {
    function __construct(){
        ...
        //frameworkinstaller-233: Required third party plugins of Membership site should point to WordPress plugin repository
        add_filter( 'wpvdemo_filter_plugin_object_required', array( $this,'wpvdemo_membership_thirdparty_plugins'), 15, 1 );
        ...
        /**
         * Framework installer 2.1.1
         * Connect old and new Views import
         */
        add_action('wpvdemo_after_views_core_import', array( $this, 'wpvdemo_import_export_views_equivalence'), 10, 1 );
        add_action('wpvdemo_after_cred_core_import', array( $this, 'wpvdemo_import_export_cred_equivalence'), 10, 1 );
        add_action('wpvdemo_after_creduserform_core_import', array( $this, 'wpvdemo_import_export_creduserform_equivalence'), 10, 1 );
        
        /**
         * Framework installer 2.1.1
         * Improved search and replace functionality by detecting URLs with add slashes in dB
         */
        add_filter( 'wpvdemo_search_replace_directive', array( $this, 'wpvdemo_searchandreplace_addslashes_urls' ), 10, 1 );
        
        /**
         * Framework installer 2.1.3 compatibility hooks for Toolset 2.4+
         */
        add_action( 'wpvdemo_before_views_import', array( $this, 'wpvdemo_disable_new_st_registration_functions_import' ), 10, 1 );
    }
...
<?php
			...
			if (! ($this->wpdemo_not_multisite ())) {
				...
				if ('yes' == $check_import_is_done) {
					...
					if ($site_is_empty) {
						...
						$refsite->actions->requisites_not_meet_msg = sprintf( __( "This demo site can't be installed. %sPlease ensure that you meet all plugin and site requirements before installing a demo site%s", "wpvdemo"),
						'<a class="wpvdemo_fi_doclink" target="_blank" href="https://wp-types.com/documentation/user-guides/using-framework-installer-to-install-reference-sites/">', '</a>.' );					
					} else {
						// Site is not empty
						// Let's show reset messages too
						$refsite->actions->requisites_not_meet_msg = sprintf( __( "This demo site can't be installed. %sPlease ensure that you meet all plugin and site requirements before installing a demo site%s To reset your Website and Database from a previous installation, you can use the reset feature under 'Toolset &ndash;&gt; Settings &ndash;&gt; Reset Demo Site", "wpvdemo"),
						'<a class="wpvdemo_fi_doclink" target="_blank" href="https://wp-types.com/documentation/user-guides/using-framework-installer-to-install-reference-sites/">', '</a>.' );
					}
				}
			} else {
    ...
    function get_refsites($refresh_check = true){
		...
		$sites_index	= wpvdemo_admin_get_sites_index( $refresh_check );
    ...
    public function wpvdemo_identify_layouts_posttype_and_import($source_dir,$refsite_file) {
    	...
    	if ((is_dir($source_dir)) &&  (method_exists($wpddlayout_theme,'layout_handle_save')))  {
    		...
    		/**
    		 * Let's import Layouts JS settings
    		 * @since 2.1.2
    		 */
    		//$wpddlayout->js_manager
    		//Define JS path inside uploads directory
    		$layouts_js_settings_path	= $source_dir . DIRECTORY_SEPARATOR . 'layouts.js';
    		
    		//Check if Layouts JS path    		
    		if ( file_exists ( $layouts_js_settings_path ) ) {
    			//exist, proceed..
    			//read contents of the setting
    			$layout_js_for_import = file_get_contents( $layouts_js_settings_path );
    			 
    			//Call Layouts core JS importing function to save this data
    			$wpddlayout->js_manager->import_js( $layout_js_for_import, true );
    			 
    			//Delete file
    			unlink( $layouts_js_settings_path );
    			 
    		}
    		/**
    		 * Let's import Layouts json settings
    		 * frameworkinstaller-284: Needed for restoring default parent layouts
    		 * @since 2.1.2
    		 */
    		//Define settings path inside uploads directory
    		$json_settings_path	= $source_dir . DIRECTORY_SEPARATOR . 'ddl-settings.json';
    		
    		//Check if json settings exists
    		
    		if ( file_exists ( $json_settings_path ) ) {
    			//exist, proceed..
    			//read contents of the setting
    			$layout_settings_for_import = file_get_contents( $json_settings_path );
    			
    			//Call Layouts core importing filter to save this data    			
    			apply_filters( 'ddl-layouts-save_imported_options', $layout_settings_for_import, TRUE );
    			
    			//Delete file
    			unlink( $json_settings_path );
...
<?php
    ...
    public function wpvdemo_if_using_wpml_three_six() {
    ...
    /**
     * Handles the import of Layouts string packages
     */
	public function wpvdemo_configure_string_packages_func() {

		$configure_string_packages = get_option( 'wpv_configure_string_packages_is_done', false );
		if ( $configure_string_packages ){
			return;
		}
		require_once WPVDEMO_ABSPATH . '/includes/import_api.php';
		...
		if ( in_array( $origin, $get_sites ) ) {
	    	...
	    	if ('yes' == $check_import_is_done) {
			    	...
			    	//Import is done but string packages are not yet updated
				    	if ( (wpvdemo_wpml_is_active() ) && ( wpvdemo_layouts_is_active() ) )
				    	{
				    		...
				    		if (defined('WPVDEMO_DOWNLOAD_URL')) {
				    			...
				    			/**
				    			 * Coming from the refsite
				    			 * @var array $string_packages_data
				    			 */
				    			...
				    			foreach ( $wpvdemo_mapped_translation_packages as $k=>$v ) {
				    				...
				    				if ((is_array( $wpvdemo_mapped_translation_packages ) ) && ( !( empty( $wpvdemo_mapped_translation_packages ) ) ) ) {
				    					...
				    					if ($key_found) {
											...
											/**
											 * Framework installer 2.1.1 - checks if the ID is implementing content layouts
											 */
											$implementing_content_layouts	=	$this->wpvdemo_implementing_content_layouts( $associated_layouts_id );
											$is_layouts_package				=	false;
											if ( isset( $pre_imported_layout_ids[ $associated_layouts_id ] ) ) {
												$is_layouts_package			=	true;
											}											
											//Step6.) Given the old Layouts ID, retrieve the imported Layouts ID
											if ( ( true === $is_layouts_package ) || ( $implementing_content_layouts > 0 ) ) {
												//Implementing a typical Layouts package or content layouts.
											   if ( true === $is_layouts_package ) {
											   		//Is a layouts package
											   		$imported_layouts_id	= $pre_imported_layout_ids[ $associated_layouts_id ];
											   } elseif ( $implementing_content_layouts > 0 ) {
											   		$imported_layouts_id	= $implementing_content_layouts;											   	
											   }
											   //Step7.) Prepare tables
											   global $wpdb;
											   ...
											   if ( ( $package_table_exist ) && ( $translation_tables_exist ) ) {
													/**
													 * At this point, package table exists
													 * 
													 */	
												   	/**
												   	 * Framework installer 2.1.1 - register content layouts detected for translation
												   	 * Register content layouts to ST
												   	 */
												   	
												   	$registered	= $this->wpvdemo_register_content_layout_for_translation( $implementing_content_layouts );
												   	
											   		//User newly generated ID.
											   		/**
											   		 * frameworkinstaller-269:
											   		 * Proceed when:
											   		 * ->Its registered (TRUE) and its content layouts
											   		 * ->Its not registered (does not matter) and its not a content layouts
											   		 */
											   		if ( ( ( true === $registered ) && ( $implementing_content_layouts > 0 ) ) || (
											   		( ( false === $registered ) && ( 0 === $implementing_content_layouts ) ) )
											   		) { 
											   			//Not registered at this stage, use.
											   			$new_package_element_id = $wpdb->get_var( 
											   							$wpdb->prepare(
											   							"SELECT ID FROM $icl_string_packages_table WHERE name = %d",
											   					$imported_layouts_id ) );
											   				
											   			/**
											   			 * Content layouts -make sure package element ID is correctly registered and it exist
											   			 */
											   			$new_package_element_id	= intval( $new_package_element_id );
											   			if ( $new_package_element_id > 0 ) {
											   				/**
											   				 * New package element ID should be registered at this point
											   				 */
												   			//Step8.) Given its new package element ID, retrieve the imported trid of this package
												   			$new_trid			= 
												   						$wpdb->get_var( 
												   						$wpdb->prepare(
												   						"SELECT trid FROM $icl_translations_table WHERE element_id = %d AND element_type='package_layout'",
												   			...
												   			if ( $new_trid_int > 0 ) {
												   				...
												   				if ( isset( $wpvdemo_mapped_translated_packages[ $old_trid ] ) ) {
												   					...
												   					$new_translation_id	= $wpdb->get_var(
												   							$wpdb->prepare(
												   								"SELECT translation_id FROM $icl_translations_table WHERE trid = %d AND source_language_code IS NOT NULL",
												   							$new_trid ) );
			    ...
			    update_option( 'wpv_configure_string_packages_is_done', true );
	    	}
		}		
	...
	/**
	 * Registers content layouts to translation after import
	 * @since 2.1.1
	 * @param number $implementing_content_layouts
	 * @return boolean
	 */
	protected function wpvdemo_register_content_layout_for_translation(  $implementing_content_layouts  = 0 ) {
		global $wpddlayout;
		$registered	= false;
		if ( ( is_object( $wpddlayout ) ) && ( $implementing_content_layouts > 0 ) ) {
			if ( method_exists( $wpddlayout, 'register_strings_for_translation' ) )	 {
				$wpddlayout->register_strings_for_translation( $implementing_content_layouts, TRUE );
				$registered	= true;
			}			
		}
		return $registered;
	...
	/**
	 * Analyze if the ID is implementing 'content layouts'
	 * @param number $id_to_analyze (ID associated with the refsite not with the imported site)
	 * @return number
	 */
	public function wpvdemo_implementing_content_layouts( $id_to_analyze = 0 ) {		
		$content_layouts_implemented	= 0;		
		if ( ( $id_to_analyze > 0 ) && ( defined( 'WPDDL_PRIVATE_LAYOUTS_IN_USE' ) ) ) {
			$using_content_layouts		= get_post_meta( $id_to_analyze, WPDDL_PRIVATE_LAYOUTS_IN_USE, true  );
			if ( 'yes' === $using_content_layouts ) {
				//Retrieved imported posts master options
				$master_imported_posts	= get_option( 'wpvdemo_processed_posts_imported' );
				if ( isset( $master_imported_posts[ $id_to_analyze ] ) ) {
					//Retrieved equivalent imported ID
					$imported_id	= $master_imported_posts[ $id_to_analyze ];
					$imported_id	= intval( $imported_id );
					if ( $imported_id > 0 ) {
						//Successfully imported, use this ID for further processing
						$content_layouts_implemented	= $imported_id;
					}
				}
					
			}			
		}
		return $content_layouts_implemented;
	}
	
	public function wpvdemo_adjust_original_id_icltranslate() {

		$adjust_original_id_icltranslate = get_option( 'wpv_adjust_original_id_icltranslate_is_done', false );
		if ( $adjust_original_id_icltranslate ){
			return;
		}
		...
		if (in_array($origin,$get_sites)) {
			...
			if ('yes' == $check_import_is_done) {
				...
				if ((wpvdemo_wpml_is_active()) && (wpvdemo_layouts_is_active()))
				{
					...
					update_option( 'wpv_adjust_original_id_icltranslate_is_done', true );
				}
			}
		}
	...
	/**
	 * Dissect refsites related information n package
	 * @param array $string_packages_data
	 * @param string $key_found
	 * @return integer
	 */
	...
	public function wpvdemo_adjust_completed_string_packages_func() {
		...
		$adjust_completed_string_packages = get_option( 'wpv_adjust_completed_string_packages_is_done', false );
		if ( $adjust_completed_string_packages ){
			return;
		}
		require_once WPVDEMO_ABSPATH . '/includes/import_api.php';
		...
		if (in_array($origin,$get_sites)) {
			...
			if ('yes' == $check_import_is_done) {					
				...
				{					
					...
					update_option( 'wpv_adjust_completed_string_packages_is_done', true );
				}		
			}
		}		
	}
    ...
    public function wpvdemo_adjust_string_packages_func() {
	    ...
	    $adjust_string_packages = get_option( 'wpv_adjust_string_packages_is_done', false );
	    if ( $adjust_string_packages ){
		    return;
	    }
    	...
    	if (in_array($origin,$get_sites)) {
	    	...
	    	if ('yes' == $check_import_is_done) {
			    ...
			    update_option( 'wpv_adjust_string_packages_is_done', true );
	    	}
    	}    
    }
    	...
    	if (isset($_POST['the_nonce_import_text']) && wp_verify_nonce($_POST['the_nonce_import_text'],
    			...
    			if ($refsite_id >0) {    		
    				...
    				$default_text_array=array(
    						/** original step 1 */
    						'activating_core_plugin' => array(
    								'the_text' 	   => 'Activating required plugins...',
    ...
    /**
     *  Required third party plugins of Membership site should point to WordPress plugin repository
     *  @since 2.0.5
     */
    public function wpvdemo_membership_thirdparty_plugins( $plugin_object ) {
    	
    	
    	if ( isset( $plugin_object->file ) ) {
    		$plugin_file = $plugin_object->file;
    		if ( ( !empty( $plugin_file ) ) && ( is_string( $plugin_file ) ) ) {    			
    			if ( 'download-manager/download-manager.php' === $plugin_file ) {
    				//Filter download manager plugin URL to point to WordPress plugin repo.
    				$plugin_object->url	= 'https://wordpress.org/plugins/download-manager/';
    			} elseif ( 'nav-menu-roles/nav-menu-roles.php' === $plugin_file ) {
    				//Filter nav menu roles URL to point to WordPress plugin repo.
    				$plugin_object->url	= 'https://wordpress.org/plugins/nav-menu-roles/';
    			}    			
    		}
    	}
    ...
    /**
     * Check if we are using Toolset 1.9 common library
     * @since 2.0.6
     */
    /**
     * Returns TRUE if using Toolset common 2.3+
     */
    public function wpvdemo_check_if_using_one_nine_common() {
    
    	$updated	= false;
    	if ( defined('TOOLSET_COMMON_VERSION') ) {
    		$common_version_using	= TOOLSET_COMMON_VERSION;
    		if ( version_compare( $common_version_using , '2.3') >= 0 ) {
    			$updated	= true;
    		} else {
    			$updated	= false;
    		}
    ...
    /**
     * In non-embedded implementation
     * Activate Types and Views to load core functions required
     * when importing a refsite.
     * @since 2.1
     */
    public function wpvdemo_activate_types_views() {
    	/**
    	 * Define main plugin file names
    	 */
    	$main_plugin_filenames			= array( 'wpcf.php', 'wp-views.php' );
    	$plugin_paths_for_activation	= array();
    	 
    	/**
    	 * Let's get the plugins in plugins directory
    	 *
    	 */
    	$all_plugins	= get_plugins();
    	/**
    	 * Let's looped through the user plugins and get the Types /Views plugin path
    	 */
    	if ( is_array( $all_plugins ) ) {
    		foreach ( $all_plugins as $plugin_path => $plugin_details ) {
    			//Get plugin file name from path
    			$plugin_filename_looped	= basename( $plugin_path );
    			if ( in_array( $plugin_filename_looped, $main_plugin_filenames ) ) {
    				//This is either Types or Views
    				//Check if its not yet added
    				if ( !in_array( $plugin_filename_looped, $plugin_paths_for_activation ) ) {
    					//Yes not yet added, add..
    					$plugin_paths_for_activation[]	= $plugin_path;
    				}
    			}
    		}
    	}
    	 
    	//Re-validated if indeed found
    	$counted	= count( $plugin_paths_for_activation );
    	 
    	/**
    	 * Activate
    	 */
    	if ( 2 === $counted ) {
    		foreach ( $plugin_paths_for_activation as $k => $plugin_to_activate ) {
    			$ret	= activate_plugin( $plugin_to_activate, '' , false, false );
    		}
    	}
    	 
    }
    /**
     * Save an option in dB for connecting old and new Views import IDs
     * @param array $views_imported_data
     * @since 2.1.1
     */
	public function wpvdemo_import_export_views_equivalence( $views_imported_data = array() ) {	
		global $wpdb;
		$views_equivalence	= array();
		if ( ( is_array( $views_imported_data ) ) && ( isset( $views_imported_data['views']['view'] ) ) ) {
			$old_views	= $views_imported_data['views']['view'];
			if ( is_array( $old_views ) ) {
				foreach ( $old_views as $k => $old_views_details ) {
					if ( isset( $old_views_details['post_name'] ) ) {
						$old_views_post_name				= $old_views_details['post_name'];
						$old_view_id						= $old_views_details['ID'];						
						$imported_views_id = $wpdb->get_var ( 
							$wpdb->prepare ( 
									"SELECT ID FROM {$wpdb->posts} WHERE post_type = 'view' AND post_name = %s", $old_views_post_name 
									) 
							);	
						$imported_views_id	= intval( $imported_views_id );
						if ( $imported_views_id > 0 ) {
							$views_equivalence[ $old_view_id ]	= $imported_views_id;
						}					
					}					
				}
				update_option('wpv_demo_views_equivalence', $views_equivalence );
			}		
		}		
	}
	/**
	 * Generate replaceable for Content Layouts after import
	 * @param array $old_ids
	 * @param array $views_equivalence
	 * @return string[][]
	 * @since 2.1.1
	 */
	public function generate_replaceable_content_layouts_import(  $old_ids = array(), 
	$views_equivalence = array(), $ct_equivalence = array(), $cred_form_equivalence = array(), $cred_user_form_equivalence = array() ) {
		$replaceable	= array();
		if ( is_array( $old_ids ) ) {
			foreach ( $old_ids as $replace_key => $old_ids_replaceables ) {
				if ( is_array( $old_ids_replaceables ) ) {
					foreach ( $old_ids_replaceables as $k => $old_id ) {
						//frameworkinstaller-299: Reset new ID to zero to force updates
						$new_id	= 0;
						if ( 'ddl_layout_view_id' === $replace_key ) {
							//We are generating Views replaceables...
							//Given the old ID, get the new ID
							if ( isset( $views_equivalence[ $old_id ] ) ) {
								$new_id			= $views_equivalence[ $old_id ];
								$new_id			= intval( $new_id );
							}
						} elseif ( 'ddl_view_template_id' === $replace_key ) {
							//We are generating Content Template replaceables...
							//Given the old ID, get the new ID
							if ( isset( $ct_equivalence[ $old_id ] ) ) {
								$new_id			= $ct_equivalence[ $old_id ];
								$new_id			= intval( $new_id );
							}							
						} elseif ( 'ddl_layout_cred_id' === $replace_key ) {
							//We are generating CRED form replaceables...
							//Given the old ID, get the new ID
							if ( isset( $cred_form_equivalence[ $old_id ] ) ) {
								$new_id			= $cred_form_equivalence[ $old_id ];
								$new_id			= intval( $new_id );
							}							
						} elseif ( 'ddl_layout_cred_user_id' === $replace_key ) {
							//We are generating CRED User form replaceables...
							//Given the old ID, get the new ID
							if ( isset( $cred_user_form_equivalence[ $old_id ] ) ) {
								$new_id			= $cred_user_form_equivalence[ $old_id ];
								$new_id			= intval( $new_id );
							}							
						}
						if ( ( isset( $new_id ) ) && ( $new_id > 0 ) ) {
							//New ID is defined, saved.
							$replaceable[]	= array(
									'srch'	=> '"'.$replace_key.'":"'.$old_id.'"',
									'rplc'	=> '"'.$replace_key.'":"'.$new_id.'"',
							);
						}
					}					
				}				
	...
	/**
	 * Save an option in dB for connecting old and new CRED import IDs
	 * @param array $cred_imported_data
	 * @since 2.1.1
	 */
	public function wpvdemo_import_export_cred_equivalence( $cred_imported_data = array() ) {
		global $wpdb;
		$xml					= simplexml_load_string ( $cred_imported_data );
		$credform_equivalence	= array();
		if ( $xml ) {			
			$import_data = wpv_admin_import_export_simplexml2array ( $xml );
			if ( ( ( is_array( $import_data ) ) ) && ( !empty( $import_data ) ) ) {
				//Get old forms
				if ( isset( $import_data['form'] ) ) {
					$old_forms	= $import_data['form'];
					if ( is_array( $old_forms ) ) {
						//Detect one form import mode
						$one_form_import_mode	= false;
						if ( ( isset( $old_forms['post_name'] ) ) && ( isset( $old_forms['ID'] ) ) ){
							$one_form_import_mode	= true;
						}
						if ( true === $one_form_import_mode ) {
							//We are importing only one form
							//Get old form name
							$old_form_name	= $old_forms['post_name'];
							//Get old form ID
							$old_form_id	= $old_forms['ID'];
							//Retrieved newly imported CRED form ID based on old form name (form name shouldn't change)
							$imported_cred_id = $wpdb->get_var (
									$wpdb->prepare (
											"SELECT ID FROM {$wpdb->posts} WHERE post_type = 'cred-form' AND post_name = %s", $old_form_name
									)
									);
							$imported_cred_id	= intval( $imported_cred_id );
							if ( ( $imported_cred_id > 0 ) && ( $old_form_id > 0 ) ) {
								//new CRED form ID found, set.
								$credform_equivalence[ $old_form_id ]	= $imported_cred_id;
							}
						} else {
							//Multiple forms imported
							//Loop through the forms
							//Loop over $oldforms and get their old form ID
							foreach ( $old_forms as $k => $old_form_details ) {
								if ( isset( $old_form_details['post_name'] ) ) {
									$old_form_name	= $old_form_details['post_name'];
									$old_form_id	= $old_form_details['ID'];
									$imported_cred_id = $wpdb->get_var (
											$wpdb->prepare (
													"SELECT ID FROM {$wpdb->posts} WHERE post_type = 'cred-form' AND post_name = %s", $old_form_name
											)
						...
						//Loop done here and all processing completed, save processed forms to options table
						update_option('wpv_demo_credform_equivalence', $credform_equivalence );
					}				
				}
			}
	...
	/**
	 * Save an option in dB for connecting old and new CRED user form import IDs
	 * @param array $creduserform_imported_data
	 * @since 2.1.1
	 */
	public function wpvdemo_import_export_creduserform_equivalence( $creduserform_imported_data = array() ) {
		global $wpdb;
		$xml						= simplexml_load_string ( $creduserform_imported_data );
		$creduserform_equivalence	= array();
		if ( $xml ) {
			$import_data = wpv_admin_import_export_simplexml2array ( $xml );
			if ( ( ( is_array( $import_data ) ) ) && ( !empty( $import_data ) ) ) {
				//Get old user forms
					...
					if ( is_array( $old_forms ) ) {
						...
						foreach ( $old_forms as $k => $old_form_details ) {
							if ( isset( $old_form_details['post_name'] ) ) {
								$old_form_name				= $old_form_details['post_name'];
								$old_form_id				= $old_form_details['ID'];
								$imported_creduserform_id	= $wpdb->get_var (
										$wpdb->prepare (
												"SELECT ID FROM {$wpdb->posts} WHERE post_type = 'cred-user-form' AND post_name = %s", $old_form_name
										)
										);
								$imported_creduserform_id	= intval( $imported_creduserform_id );
								if ( ( $imported_creduserform_id > 0 ) && ( $old_form_id > 0 ) ) {
									$creduserform_equivalence[ $old_form_id ]	= $imported_creduserform_id;
								}
							}
						}
						update_option('wpv_demo_creduserform_equivalence', $creduserform_equivalence );
					}
				}
			}
	...
	/**
	 * Original replaceables does not have added back slashes
	 * In escaped URLs in dB, URls are escaped
	 * We want to generate a comptabible replaceable to cover this case
	 * @param array $replaceables
	 * @return array
	 */
	public function wpvdemo_searchandreplace_addslashes_urls( $replaceables = array() ) {
		
		//Loop over replaceables
		$updated_replaceables	= $replaceables;
		if ( is_array( $replaceables ) ) {
			foreach ( $replaceables as $k => $v ) {
				if ( isset( $v['srch'] ) ) {
					$current_srch	= $v['srch'];
					$updated_srch	= str_replace('/', '\/', $current_srch );			
				} 
				if ( isset( $v['rplc'] ) ) {
					$current_rplc	= $v['rplc'];
					$updated_rplc	= str_replace('/', '\/', $current_rplc  );
				}
				$updated_data	= array( 
								'srch' 	=> $updated_srch, 
								'rplc'	=> $updated_rplc						
				);	
				$updated_replaceables[]	= $updated_data;
			}
		}
		return $updated_replaceables;
		
	}
	
	/**
	 * Temporarily disable string registration functions from Views import while FI imports a site.
	 * To prevent any errors because of incomplete data.
	 * @param array $import_data
	 */
	public function wpvdemo_disable_new_st_registration_functions_import( $import_data = array() ) {		
		//Check if we doing a multilingual import
		if ( wpvdemo_wpml_is_enabled() ) {
			//Yes, we get an instance of WPV_WPML_Integration
			if ( ( class_exists( 'WPV_WPML_Integration_Embedded' ) ) && ( method_exists( 'WPV_WPML_Integration_Embedded', 'get_instance' ) ) ) {
				$wpv_wpml_integration_instance	= WPV_WPML_Integration_Embedded::get_instance();
				if ( is_object( $wpv_wpml_integration_instance ) ) {
					//We only remove this hook if import is not done yet
					$check_import_is_done = get_option ( 'wpv_import_is_done' );
					if ('yes' != $check_import_is_done ) {
						// Import not yet done
						remove_all_actions( 'wpv_action_wpv_register_wpml_strings' );
					}					
				}
			}
