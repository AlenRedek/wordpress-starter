...
function wpvdemo_admin_get_sites_index($refresh_check = true) {
    ...
    if(defined('WPVDEMO_DEBUG') && WPVDEMO_DEBUG) {
    	...
    	/**
    	 * Fires when sites indexed are defined
    	 */
    	do_action( 'wpvdemo_sites_index_loaded', $sites_index );
    	
    	/**
    	 * Return with options for filtering.
    	 */
...
function wpvdemo_activate_plugins($plugins,$activated_plugins_siteshortname) {
	...
	foreach ($plugins_array_forchecking as $key=>$value) {
		...
		if (!$required_active) {
			...
			if ($available) {     			
     			if ( 'download-manager/download-manager.php' == $available['file'] ) {
     				$success = activate_plugin($available['file'], null, false, true );
     			} else {
...
function wpvdemo_is_allowed_plugin($plugin,$plugin_element=array()) {
	...
	global $frameworkinstaller;	
	$original_exclusion	= array( 'wordpress-importer.php', 'w3-total-cache.php', 'views-demo.php' );
...
function wpvdemo_handle_dd_layouts_settings_custom_replacement( $active_site_layouts, $original_host, $migration_redesigned_layouts_hostname, $parsed_host ) {
	...
	/**
	 * Proceed only if Layouts meta keys set
	 */
	if ( defined( 'WPDDL_LAYOUTS_SETTINGS') ) { 
		$ddlayouts_meta_key	= WPDDL_LAYOUTS_SETTINGS;
		...
		WHERE meta_key='$ddlayouts_meta_key' AND meta_value LIKE %s
		";
			...
			*/
			...
			if ((is_array($final_layouts_for_processing)) && (!(empty($final_layouts_for_processing)))) {
				...
				foreach ($final_layouts_for_processing as $k=>$id_for_processing) {
					...
					$meta_value_serialized = get_post_meta( $id_for_processing, $ddlayouts_meta_key, TRUE );
...
function wpvdemo_plugin_plugin_row_meta($plugin_meta, $plugin_file, $plugin_data, $status) {
	...
	/**
	 * frameworkinstaller-286: Don't process if server does not have cURL
	 */
	if ( ( defined('WPVDEMO_ABSPATH') ) && ( defined('WPVDEMO_VERSION') ) && ( is_callable('curl_init') ) ) {
		if ( ( WPVDEMO_ABSPATH ) && ( WPVDEMO_VERSION ) ) {
			$this_plugin = basename( WPVDEMO_ABSPATH ) . '/views-demo.php';
			if ( $plugin_file == $this_plugin ) {
				//This is framework intaller
				$version_slug = 'framework-installer-';
...
/**
 * Download a zip file given a source refsite URL
 * To a target path say uploads directory
 * Then extract it to that path and delete that zip afterwards.
 * @param string $url the source refsite zipped URL, e.g. http://ref.wp-types.tld/_wpv_demo/membership/download-manager-files.zip
 * @param @download_path string the target extract directory, e.g. /home/emerson/sourcecode/wptest/wp-content/uploads/download-manager-files
 */
function wpvdemo_download_anything( $url, $download_path ) {

	//Define source refsite URL
	$file				= $url;
	
	// Define the extraction path to local
	$extraction_path	= dirname( $download_path );

	// Retrieved file headers
	$file_headers_zip = @get_headers ( $file );

	if ( is_dir ( $download_path ) ) {
		//Directory exist
		// Make sure we can delete			
		chmod_R ( $download_path , 0777, 0777 );
				
		// Now delete
		rmdir_recursive ( $download_path );		
	}
	// Download zip file
	if (strpos ( $file_headers_zip [0], '200 OK' )) {
			//Resource exist in server
			// Set context
			...
			$new_file	= $extraction_path.DIRECTORY_SEPARATOR.$zip_file_name;
...
/**
 * When Views refsites are unpublished, detect it automatically.
 * This will allow a Framework installer with both forward and backward compatibility.
 * No need to update codes when these are changed in the refsites API.
 * @param array $sites_index
 */
function wpvdemo_detect_if_views_refsites_deactivated( $sites_index ) {
	
	global $wpvdemo_disable_views_versions;
	
	//Default to false
	$wpvdemo_disable_views_versions	= false;
	$disabled_checks				= array();
	/**
	 * Attempt to get cache
	 * 
	 */
	$published_refsites = wp_cache_get( 'wpvdemo_sites_arrays_published' );
	if ( false === $published_refsites ) {
		/**
		 * Cached not set
		 * Define values
		 */
		$sites							= wpvdemo_get_sites_index_as_arrays( $sites_index );
		...
		foreach ( $sites as $k=>$v ) {
			...
			if ( $site_id_analyzed > 0 ) {
				$published_refsites[]	= $site_id_analyzed;
			}
		 ...
		 * Store to cache
		 */
		wp_cache_set( 'wpvdemo_sites_arrays_published', $published_refsites );
	}
	
	$disabled	= array(8, 48, 53, 59, 65 );
	
	if ( !empty( $published_refsites ) ) {
		//Loop over published sites and check if Views version are disabled
		foreach ( $disabled as $k_disabled => $refsite_disabled_id ) {
			if ( !in_array( $refsite_disabled_id, $published_refsites ) ) {
				//Nope not anymore published,
				$disabled_checks[]	= $refsite_disabled_id;
			}
		}		
	}
	 ...
	 * Assessment
	 */
	if ( $disabled	== $disabled_checks ) {
		//All disabled
		$wpvdemo_disable_views_versions	= true;
	}
}
/**
 * Search and array recursively and return the values
 * @since 2.1.1
 * @param array $array
 * @param string $key
 * @param string $value
 * @return array[]
 */
function wpvdemo_recursive_array_search($array = array(), $key = '')
{
	$results = array();
	...
	if (is_array( $array ) ) {
		...
		foreach ( $array as $subarray ) {
			$results = array_merge( $results, wpvdemo_recursive_array_search( $subarray, $key ) );
		}
	}
...
/**
 * Search and replace function using Framework_Installer_Migration_Search_Replace class
 * @param array $replaceable
 * $replaceable should be formatted like this:
 * $replace = array( 0 => array( srch => 'old_data', rplc => 'new_data ), 1 => array( srch => 'old_data', rplc => 'new_data ) ... )
 * @since 2.1.1
 */
function wpvdemo_universal_search_and_replace_function( $replaceable = array() ) {
...
/**
 * Disable CRED post expiration event while import is not done.
 */
function wpvdemo_disable_wp_cron_import_ongoing() {
	$check_import_is_done = get_option ( 'wpv_import_is_done' );
	if ('yes' != $check_import_is_done ) {
		// Import not yet done
		global $cred_post_expiration;
		if ( ( is_object( $cred_post_expiration ) ) && ( ( wpvdemo_wpml_is_enabled() ) ) && ( !is_multisite() ) ) {
			//Remove this hook only when having a multilingual imports on standalone site.
			remove_action( 'cred_post_expiration_event', array( $cred_post_expiration, 'cred_pe_schedule_event_action' ), 11 );
		}
	}
}
 ...
 */
function wpvdemo_custom_delete_post_type( $post_type_slug ) {
 ...
 */
function wpvdemo_custom_delete_tax( $tax_slug ) {
 ...
 */
function wpvdemo_custom_delete_field_group( $group_slug ) {
 ...
 */
function wpvdemo_custom_delete_fields( $field_slugs ) {
