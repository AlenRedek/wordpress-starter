...
function wpvdemo_import_views($baseurl, $settings) {
	...
	do_action( 'wpvdemo_before_views_import', $import_data );
	...
	$wpvdemo_import->process_wpml ( 'views' );
	...
	/**
	 * Hooks for firing when Views are already imported
	 */
	do_action( 'wpvdemo_after_views_core_import', $import_data );
	...
	if (defined ( 'CRED_FE_VERSION' )) {
		...
		if (in_array($cred_evaluate_shortname,$sites_requiring_cred_commerce)) {			
			...
			if (! (empty ( $cred_commerce_path_required ))) {
				...
				$result = cred_import_xml_from_string ( $data, $cred_import_options );
		...
		/**
		 * Hooks for firing when CRED are already imported
		 */
		do_action( 'wpvdemo_after_cred_core_import', $data );
		
		/**
		 * Here we import sites requiring CRED user forms
		 * 
		 */
		...
		if (in_array( $cred_evaluate_shortname , $sites_requiring_cred_user_forms ) ) {
			//This refsite requires CRED User forms, proceed to import.
			...
			/**
			 * Hooks for firing when CRED user forms are already imported
			 */
			do_action( 'wpvdemo_after_creduserform_core_import', $cred_user_form_exported_data );
		}
	}
 ...
 */
function wpvdemo_import_access($baseurl, $settings, $overwrite = false ) {
	global $wpdb;
	...
	if (defined ( 'TACCESS_VERSION' ) && function_exists ( 'taccess_import' )) {
		...
		if ( true === $overwrite ) {
			//Needed for Access re-import, overwrite settings
			//Pass overwrite argument and set to true
			$options_access['access-overwrite-existing-settings'] = true; 
		}
