...
function wpvdemo_import($site_id, $step,$versions='') {
    ...
    if ( defined( 'WPCF_EMBEDDED_INC_ABSPATH' ) ) {
    ...
    require_once WPVDEMO_ABSPATH . '/includes/inprocess-import-hooks.php';
    ...
    switch ($step) {
    	case 1:
    		global $frameworkinstaller;
    		$frameworkinstaller->wpvdemo_activate_types_views();
    		break;
        case 2:     
            ...
            if ( empty( $versions ) ) {
            	$versions = get_option('wpvdemo_the_version_installed' );
            }
        ...
        case 12:
        		...
        		if (!(array_key_exists($bootmagsite_title,$bootstrap_sites_shortname_array))) {
	        		...
	        		if (!empty($settings->theme_mods)) {
	        			...
	        			foreach ($settings->theme_mods as $mods) {
	        				foreach ($mods as $mod_name => $mod_value) {
						        $mod_value_string = (string)$mod_value;
						        if(  trim( $mod_value_string ) != '' ) {
							        $mod_value                         = wpvdemo_convert_url( (string) $mod_value, $settings );
							        $update_mods[ (string) $mod_name ] = $mod_value;
						        }else{
							        $update_inner_mods = array();
							        foreach ( $mod_value as $mod_inner_name => $mod_inner_value ) {
								        $mod_inner_value                         = wpvdemo_convert_url( (string) $mod_inner_value, $settings );
								        $update_inner_mods[ (string) $mod_inner_name ] = $mod_inner_value;
							        }
							        if ( empty( $update_inner_mods ) ) {
								        $update_inner_mods = false;
							        }
							        $update_mods[ (string) $mod_name ] = $update_inner_mods;
						        }
	        				}
	        			}
	        ...
	        if ( $settings->shortname == 'gswt' ){
		        wpvdemo_custom_delete_post_type( 'consultant' );
		        wpvdemo_custom_delete_tax( 'language' );
		        wpvdemo_custom_delete_field_group( 'consultant-details');
		        $fields_to_remove = array( 'hourly-rate', 'site-visits', 'expertise', 'consultant-phone', 'consultant-email', 'profile-image' );
		        wpvdemo_custom_delete_fields( $fields_to_remove );
	        }
...
require_once WPVDEMO_ABSPATH . '/includes/main_functions/inprocess-import-functions.php';
