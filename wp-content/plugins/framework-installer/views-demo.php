...
/*
  ...
  Version: 2.1.7.2
 */
define('WPVDEMO_VERSION', '2.1.7.2');
...
if ( defined('WPVDEMO_DEBUG') ) {
	if ( true === WPVDEMO_DEBUG ) {
		/**
		 * Framework installer debug mode enabled
		 * Automatically load canonical sites only
		 */
		if (!defined('WPVDEMO_CANONICAL_SITES') ) {
			define('WPVDEMO_CANONICAL_SITES', true );
		}	
	}
}
...
add_action('plugins_loaded', 'wpvdemo_admin_get_sites_index', 1);
...
add_action( 'wpvdemo_sites_index_loaded','wpvdemo_detect_if_views_refsites_deactivated', 1, 1 );
add_action( 'plugins_loaded','wpvdemo_disable_wp_cron_import_ongoing', 0 );
