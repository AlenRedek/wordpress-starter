...
class WPDD_layout_tabs_pane extends WPDD_layout_container_row{
    ...
    function render_tabs( $renderer ){
      /*  if( $this->navigation_style === 'tabs' ){
        ...
        $return = apply_filters('ddl-tab_get_tab_pane_element_open', '<div role="tabpanel" class="'.esc_attr( $tabClass ).' '.esc_attr( $active ). ' '. esc_attr( $fade ).'" id="'.esc_attr( $this->get_anchor() ).'" '.$tab_data_attributes.' >', $this, WPDD_layouts_layout_tabs::$tab_count);
