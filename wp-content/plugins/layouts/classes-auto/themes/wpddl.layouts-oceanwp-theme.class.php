...
class Toolset_Compatibility_Theme_oceanwp extends Toolset_Compatibility_Theme_Handler {
    ...
    public function add_register_styles( $styles ) {
        ...
        $styles['oceanwp-overrides-css'] = new WPDDL_style( 'oceanwp-overrides-css', WPDDL_RES_RELPATH . '/css/themes/oceanwp-overrides.css', array(), WPDDL_VERSION, 'screen' );
	...
	public function disable_featured_image() {
		...
		if ( $this->get_toolset_theme_option('toolset_disable_featured_image') ) {
			add_action( 'wp_head', array( $this, 'remove_featured_image' ) );
			return true;
		}
		return false;
	}
	...
	public function remove_featured_image(){
		echo '<script type="text/javascript">jQuery(function() { jQuery(".single .thumbnail").remove(); });</script>';
	}
	...
	public function disable_pagination() {
		...
		if ( $this->get_toolset_theme_option( 'toolset_disable_pagination' ) ) {
			add_action( 'wp_head', array( $this, 'remove_pagination' ) );
			return true;
		}
		return false;
	}
	...
	public function remove_pagination(){
		echo '<script type="text/javascript">jQuery(function() { jQuery(".oceanwp-pagination").remove(); });</script>';
	}
	...
	public function disable_archive_title(){
		...
		if ( $this->get_toolset_theme_option( 'toolset_disable_title' ) && ( is_archive() || is_page() || $this->is_custom_post_type() ) ) {
	...
	public function disable_title() {
		...
		if ( $this->get_toolset_theme_option( 'toolset_disable_title' ) && ( is_single() || is_page() ) ) {
	...
	public function remove_title( ) {
		echo '<script type="text/javascript">jQuery(function() { jQuery(".entry-header .single-post-title.entry-title").remove(); });</script>';
	}
